...
function init() {
    APP_KEY="com.sankuai.travel.dsg.mscadvancepay"
    DEPLOY_ARTIFACT_ID="msc-advance-pay-service"
    ...
    if [ -z "$WORK_PATH" ]; then
        WORK_PATH="/opt/meituan/mobile/$APP_KEY"
    fi
...
function run() {
    ...
    deployenv=$(grep 'deployenv=' /data/webapps/appenv | cut -b  11-);
    echo $deployenv
    EXEC_JAVA=$EXEC_JAVA" -Ddeployenv=$deployenv"
